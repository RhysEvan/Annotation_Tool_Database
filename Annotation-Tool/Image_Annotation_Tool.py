...
from skimage.draw import disk, polygon2mask
...
class Annotate():
    ...
    def __init__(self, exclude_labeled=True, run=True, bands=[1]):
        ...
        if exclude_labeled and '.jpg' in filenames[0]:
            filenames = [fn for fn in filenames if not os.path.exists(fn.replace(".jpg", "_label.tif"))]
            filenames = [fn for fn in filenames if fn.find("_label.tif") == -1]
        if exclude_labeled and '.png' in filenames[0]:
            filenames = [fn for fn in filenames if not os.path.exists(fn.replace(".png", "_label.tif"))]
            filenames = [fn for fn in filenames if fn.find("_label.tif") == -1]
        if exclude_labeled and '.tif' in filenames[0]:
            filenames = [fn for fn in filenames if not os.path.exists(fn.replace(".tif", "_label.tif"))]
            filenames = [fn for fn in filenames if fn.find("_label.tif") == -1]
        if len(filenames) == 0:
            print("No images")
            return
    ...
    def zoom(self, step):
        ...
        xdata = step.xdata # get event x location
    ...
    def gen_index_image(self, im, poly): #Makes a new np.array and fills the areas that were annotated with a polygon with 1, the rest with 0
        ...
        for labels in poly:
            ...
            if len(poly) == 1:
                ...
                mask = disk((int(poly_coor[0, 0]), int(poly_coor[0, 1])), 10)
...
def save_labeled(fn, label_im):
    if ".jpg" in fn.lower() or ".tif" in fn.lower():
        ...
        fn_out = fn_out.replace('.jpg', '_label.tiff')
